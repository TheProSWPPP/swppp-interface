...
**Architecture:** Builds on Plan 1's `permit_facilities` (`status='promoted'` rows are the input). A pure TCEQ parser + a fetch wrapper feed an enrichment runner that writes a new `permit_enrichment` table and advances facility status to `enriched`. A pure CSV builder + a download endpoint produce the direct-mail file. A new "Enrichment" sub-view in the existing Permits tab drives it.

**Tech Stack:** Node ESM + Express + `pg`, React + TypeScript + Tailwind + lucide-react, Vitest (pure logic). **TCEQ transport: plain HTTPS `fetch` from the Railway backend (primary — the wq_dpa page is server-rendered HTML, no browser needed); Browserless `/content` + residential proxy only as a fallback if TCEQ blocks Railway's (GCP) IP range.** Task 1 decides which.
...
- TCEQ detail URL: `https://www2.tceq.texas.gov/wq_dpa/index.cfm?fuseaction=home.validate_permit&permit_number=<PERMIT>` (server-rendered HTML, **no JS/browser needed** — a plain `fetch` gets it). TCEQ blocks the dev sandbox at TCP level; a US datacenter IP (Firecrawl) reached it. Whether Railway's GCP IP is blocked is unknown until Task 1.
- Browserless **fallback only** (if Railway IP is blocked): token from the CMD-scraper workflow, params `proxy=residential`, `blockConsentModals=true`. Store as env `BROWSERLESS_TOKEN`; never hardcode. Not needed if plain fetch works.
...
## Task 1: Decide the TCEQ transport (plain fetch first)

Determines whether a free plain `fetch` from Railway reaches TCEQ, or whether the Browserless fallback is needed. Decide before building the fetch wrapper.

**Files:** none (verification only).

- [ ] **Step 1: Test a plain fetch from the Railway backend (US/GCP IP)**

The dev sandbox is TCP-blocked by TCEQ, so this must run from Railway, not locally. Use a Railway one-off command (or a temporary route) to fetch one permit page:
```bash
railway run --service swppp-interface node -e 'fetch("https://www2.tceq.texas.gov/wq_dpa/index.cfm?fuseaction=home.validate_permit&permit_number=TXR05DP22").then(r=>r.text()).then(t=>console.log("len",t.length,/Summary of Authorization/i.test(t)?"REACHED":"blocked-or-empty")).catch(e=>console.log("ERR",e.message))'
```
(If `railway run` isn't available in this context, add a temporary `GET /api/permits/_tceq-probe` route that does the same fetch and returns the result, hit it once, then remove it.)
Expected: `REACHED` → plain fetch works, **no Browserless needed**. `ERR`/`blocked` → use the Browserless fallback.

- [ ] **Step 2: Record the decision**

Note the outcome in Task 3's commit message. If plain fetch works (likely), Task 3 uses `fetch` directly and `BROWSERLESS_TOKEN` is never required. If blocked, set `BROWSERLESS_TOKEN` on Railway and Task 3 routes through Browserless. No code commit in this task.
...
Plain `fetch` is the default path; Browserless is used only if `BROWSERLESS_TOKEN` is set (the fallback wired in per Task 1's outcome).
...
- [ ] **Step 2: Verify it parses; live smoke test from a US host**

Run: `node --check lib/permitTceqFetch.js` (exit 0).
The dev sandbox is TCP-blocked by TCEQ, so the live smoke test must run from Railway (plain-fetch path) — same mechanism as Task 1's probe — or, if using the Browserless fallback, it can run from the controller with `BROWSERLESS_TOKEN` set:
```bash
...
```
Expected: non-zero length + `contact-found` (from whichever transport Task 1 selected).

- [ ] **Step 3: Commit** (record in the message which transport Task 1 selected)

```bash
git add lib/permitTceqFetch.js
git commit -m "feat(permits): TCEQ fetch (plain HTTPS primary, Browserless residential fallback)
