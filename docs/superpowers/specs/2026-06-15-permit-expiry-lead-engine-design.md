...
- **NEC holders (~3,846)** = certified "no exposure," exempt from SWPPP → stored + flagged, not targeted this phase.
...
The Permits tab shows **"today's remaining send budget"** so the ceiling is always visible.
...
### In scope (Phase 4)
- EPA Envirofacts ingest for TXR050000 (one-shot + monthly re-pull)
...
- **NEC holders** as a real campaign — stored + flagged, not targeted.
