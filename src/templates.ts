export interface StateTemplate {
  id: string;
  name: string;
  documents: string[];
  templateLinks?: Record<string, string>;
}

export const STATE_TEMPLATES: StateTemplate[] = [
  {
    id: "68e7f665af102d93f751e93f",
    name: "Pennsylvania",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
    },
  },
  {
    id: "67b7115f07a4de27d6560d7c",
    name: "South Dakota",
    documents: ["SWPPP", "Job Cover Letter", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      NOI: "#",
    },
  },
  {
    id: "6751c1aa26054e07f6fb32c2",
    name: "Kentucky",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
    },
  },
  {
    id: "6717db7dad39071ce06debfa",
    name: "Idaho",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI",
      "NOT",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
      NOT: "#",
    },
  },
  {
    id: "6717bc827a507b3bc114a869",
    name: "Nebraska",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI",
      "NOT",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
      NOT: "#",
    },
  },
  {
    id: "67111b5c8ae11012df549b42",
    name: "Texas Large",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI",
      "NOT",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
      NOT: "#",
    },
  },
  {
    id: "66e30eb53756a712d2496ea8",
    name: "Nevada",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI",
      "NOT",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
      NOT: "#",
    },
  },
  {
    id: "66e1b1acab871ae3b9d60003",
    name: "Indiana",
    documents: ["SWPPP", "Job Cover Letter", "NOI (IDEM)"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "NOI (IDEM)": "#",
    },
  },
  {
    id: "66685b9296be15f4f0636667",
    name: "Wyoming",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
    },
  },
  {
    id: "6660a16934a239ee65530a59",
    name: "North Dakota",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
    },
  },
  {
    id: "6659273cc25b77f93f4a1a7f",
    name: "Ohio",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
    },
  },
  {
    id: "65d4ca3a02598b72ab2fd9c6",
    name: "Mississippi Large",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI Packet",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      "NOI Packet": "#",
    },
  },
  {
    id: "6541b076598234cb67205a81",
    name: "Virginia",
    documents: ["SWPPP", "Job Cover Letter", "Registration Statement"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Registration Statement": "#",
    },
  },
  {
    id: "6541af58e95df0a6f77802e8",
    name: "North Carolina",
    documents: ["SWPPP", "Job Cover Letter", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      NOI: "#",
    },
  },
  {
    id: "6541acb5a7b779c0ddb59920",
    name: "Illinois",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "NOI (CDX Portal)",
      "EcoCat Letter",
      "SHPO Letter",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "NOI (CDX Portal)": "#",
      "EcoCat Letter": "#",
      "SHPO Letter": "#",
    },
  },
  {
    id: "65284bbb802560e6c4c0e693",
    name: "Tennessee",
    documents: ["SWPPP", "Job Cover Letter", "NOI PDF Form", "NOT PDF Form"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "NOI PDF Form": "#",
      "NOT PDF Form": "#",
    },
  },
  {
    id: "6512a695b32b8ea85e86fe1d",
    name: "Missouri",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI (MOGEM)",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      "NOI (MOGEM)": "#",
    },
  },
  {
    id: "6512a4761c0734188c7680ff",
    name: "New Mexico",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI/LREW (CDX Portal)",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      "NOI/LREW (CDX Portal)": "#",
    },
  },
  {
    id: "6512a437b871e632560e1545",
    name: "New York",
    documents: ["SWPPP", "Job Cover Letter", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      NOI: "#",
    },
  },
  {
    id: "6512a0af73c2ec06946e3f1a",
    name: "Utah",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI/LREW (CDX Portal)",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      "NOI/LREW (CDX Portal)": "#",
    },
  },
  {
    id: "6512936e68837bf90d658eba",
    name: "Georgia",
    documents: ["SWPPP", "Job Cover Letter", "NOI (GEOS)"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "NOI (GEOS)": "#",
    },
  },
  {
    id: "64cd2733aeed6b8d990806ea",
    name: "Alabama",
    documents: ["SWPPP", "Job Cover Letter", "NOI", "USGS Topo Map"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      NOI: "#",
      "USGS Topo Map": "#",
    },
  },
  {
    id: "64ca92fc0f053b9e3f04c368",
    name: "Arkansas Small",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
    },
  },
  {
    id: "64c031eb140e1cf59455153e",
    name: "Louisiana Large",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI",
      "NOT",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
      NOT: "#",
    },
  },
  {
    id: "64becfb6fca4026911948fdf",
    name: "Louisiana Small",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "SCACR Form",
      "SCACR LDEQ",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      "SCACR Form": "#",
      "SCACR LDEQ": "#",
    },
  },
  {
    id: "64b6b858e75c608e42fa1a83",
    name: "South Carolina",
    documents: ["SWPPP", "Job Cover Letter", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      NOI: "#",
    },
  },
  {
    id: "64a852a934449c11316e36d4",
    name: "Colorado",
    documents: [
      "SWPPP",
      "Job Cover Letter",
      "Construction Site Notice",
      "NOI (CEOS Portal)",
    ],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      "NOI (CEOS Portal)": "#",
    },
  },
  {
    id: "64a852634ca26165d8e0fd7c",
    name: "Arizona",
    documents: ["SWPPP", "Job Cover Letter", "NOI (AZDEQ portal)"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "NOI (AZDEQ portal)": "#",
    },
  },
  {
    id: "64a85225f20295c08c40ceb5",
    name: "Arkansas Large",
    documents: ["SWPPP", "Job Cover Letter", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      NOI: "#",
    },
  },
  {
    id: "64a85159def12cfae605dfc7",
    name: "Florida",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice", "NOI"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
      NOI: "#",
    },
  },
  {
    id: "64a848de70888d805f4d5097",
    name: "Oklahoma",
    documents: ["SWPPP", "Job Cover Letter", "NOI PDF Form", "NOT PDF Form"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "NOI PDF Form": "#",
      "NOT PDF Form": "#",
    },
  },
  {
    id: "64a5d4a9b39dd4b060d9cb07",
    name: "Texas Small",
    documents: ["SWPPP", "Job Cover Letter", "Construction Site Notice"],
    templateLinks: {
      SWPPP: "#",
      "Job Cover Letter": "#",
      "Construction Site Notice": "#",
    },
  },
];

export const DEFAULT_DOCUMENTS = [
  "SWPPP",
  "Job Cover Letter",
  "Construction Site Notice",
];

export function getDocumentsForTemplate(templateIdOrName?: string): string[] {
  if (!templateIdOrName) return DEFAULT_DOCUMENTS;

  const template = STATE_TEMPLATES.find(
    (t) => t.id === templateIdOrName || t.name === templateIdOrName
  );

  return template ? template.documents : DEFAULT_DOCUMENTS;
}

export function getTemplateDocLink(
  templateIdOrName: string | undefined,
  docName: string
): string | undefined {
  // Special case for Job Order PDF - universal across all templates
  if (docName === "Job Order PDF") {
    return "#"; // Placeholder for Job Order PDF template
  }

  if (!templateIdOrName) return undefined;
  const template = STATE_TEMPLATES.find(
    (t) => t.id === templateIdOrName || t.name === templateIdOrName
  );
  return template?.templateLinks?.[docName];
}

export function getTemplateName(templateIdOrName?: string): string | undefined {
  if (!templateIdOrName) return undefined;
  const template = STATE_TEMPLATES.find(
    (t) => t.id === templateIdOrName || t.name === templateIdOrName
  );
  return template ? template.name : templateIdOrName;
}

const colors = [
  "bg-blue-50 text-blue-700 border-blue-200",
  "bg-purple-50 text-purple-700 border-purple-200",
  "bg-brand-50 text-brand-700 border-brand-200",
  "bg-pink-50 text-pink-700 border-pink-200",
  "bg-rose-50 text-rose-700 border-rose-200",
  "bg-orange-50 text-orange-700 border-orange-200",
  "bg-amber-50 text-amber-700 border-amber-200",
  "bg-lime-50 text-lime-700 border-lime-200",
  "bg-green-50 text-green-700 border-green-200",
  "bg-emerald-50 text-emerald-700 border-emerald-200",
  "bg-teal-50 text-teal-700 border-teal-200",
  "bg-cyan-50 text-cyan-700 border-cyan-200",
  "bg-sky-50 text-sky-700 border-sky-200",
];

export function getTemplateColor(templateIdOrName?: string): string {
  const name = getTemplateName(templateIdOrName);
  if (!name) return "bg-slate-50 text-slate-700 border-slate-200";
  let hash = 0;
  for (let i = 0; i < name.length; i++) {
    hash = name.charCodeAt(i) + ((hash << 5) - hash);
  }
  const index = Math.abs(hash) % colors.length;
  return colors[index];
}

export function isAutomatedDocument(docName: string): boolean {
  const automated = [
    "Job Cover Letter",
    "Construction Site Notice",
    "Job Order PDF",
  ];
  return automated.includes(docName);
}
