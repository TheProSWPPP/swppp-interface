...
export function registerNurtureRoutes(app, pool) {
  ...
  app.post("/api/sdr/nurture/campaigns/:id/test", async (req, res) => {
    ...
    // test sends are non-destructive → no admin gate. Compute → audit → respond (match the other handlers; never respond before the try can fail).
    try { const r = await brevo.sendCampaignTest(req.params.id, emailTo); await audit(req, "campaign.test", "campaign", req.params.id, `test → ${emailTo.join(",")}`); res.json(r || { ok: true }); }
  ...
  app.patch("/api/sdr/nurture/campaigns/:id/schedule", async (req, res) => {
    if (!guard(res)) return;
    if (!requireAdmin(req, res)) return; // scheduling queues a real send to real contacts — same blast radius as send
  ...
  app.post("/api/sdr/nurture/campaigns/:id/suspend", async (req, res) => {
    if (!guard(res)) return;
    if (!requireAdmin(req, res)) return; // cancelling a scheduled send is a send-state mutation → admin only
  ...
  app.post("/api/sdr/nurture/campaigns/:id/duplicate", async (req, res) => {
    if (!guard(res)) return;
    // Admin only. This creates a Brevo campaign carrying caller-supplied listIds AND a
    // scheduledAt, so without a gate it reached the same blast radius as /send and /schedule
    // while sitting behind neither of their gates.
    if (!requireAdmin(req, res)) return;
