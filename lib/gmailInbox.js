...
// Reply text: the user's note, then the original quoted "> " line by line, like every mail client.
export function buildReplyText(note, msg) {
  const quoted = (msg?.body || "").split("\n").map((l) => `> ${l}`).join("\n");
  return `${note || ""}\n\nOn ${msg?.date || ""}, ${msg?.from || ""} wrote:\n${quoted}`;
}

export function buildForwardText(note, msg) {
  return `${note || ""}\n\n---------- Forwarded message ----------\n${fwdHeaderLines(msg).join("\n")}\n\n${msg?.body || ""}`;
}

// Forward html: the user's note on top, then the original message's real HTML (or a
// pre-wrapped text fallback) inside a quoted block, so formatting/signature survive.
export function buildForwardHtml(note, msg) {
