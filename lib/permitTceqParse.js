...
function parseListPage(html, permit) {
  ...
  for (const row of rows) {
    ...
    // first DATA row = the one whose permit-# cell matches (skip the header row)
    if (cells[0] && cells[0].toUpperCase().includes((permit || "").toUpperCase()) && cells[0].toLowerCase() !== "permit #") {
