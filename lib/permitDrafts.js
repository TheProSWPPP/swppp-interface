...
// Existing-customer check, shared by draft generation and the send choke point.
// Indexes ONLY data->>'companyName' (the company the work was for). projects.name is the
// PROJECT title and would add 423 junk keys ("kfc", "circlek", "home", "sand pit"), any of
// which could permanently kill a real operator — permit_outreach has no delete path.
let _custKeys = null;
let _custKeysAt = 0;
const CUST_TTL_MS = 5 * 60 * 1000;
async function customerKeySet(pool) {
  if (_custKeys && Date.now() - _custKeysAt < CUST_TTL_MS) return _custKeys;
  try {
    const { rows } = await pool.query(
      `SELECT data->>'companyName' AS company FROM projects
        WHERE archived = false AND data->>'companyName' IS NOT NULL`,
    );
    _custKeys = crossRefIndex(rows, (r) => r.company);
    _custKeysAt = Date.now();
  } catch (e) {
    console.error("[permit-drafts] customer cross-ref failed:", e.message);
    return new Set(); // fail open, never block on a cross-ref outage
  }
  return _custKeys;
}
async function isExistingCustomer(pool, operatorName) {
  const k = companyKey(operatorName);
  if (!k) return false;
  return (await customerKeySet(pool)).has(k);
}
  ...
  const { rows: ops } = await pool.query(
    `SELECT e.operator_key, e.operator_name, e.email, e.contact_name,
      ...
      WHERE e.email IS NOT NULL AND e.email <> ''
        ...
        AND NOT EXISTS (SELECT 1 FROM permit_outreach o2
                         WHERE o2.operator_key = e.operator_key AND o2.status = 'skipped'
                           AND o2.note LIKE 'auto: skipped existing customer%')
...
export async function approveAndSendPermitDraft(pool, id) {
  ...
  // Existing customer: refuse at the send choke point, whichever path produced the draft.
  // lib/permitAuto.js filters its own candidates, but a draft can also be generated and
  // approved by hand, so the last gate before Apollo has to check too.
  if (await isExistingCustomer(pool, draft.operator_name)) {
    await pool.query(
      `UPDATE permit_drafts SET status = 'rejected',
              reject_reason = 'auto: existing customer', updated_at = NOW() WHERE id = $1`,
      [id],
    );
    throw httpError(409, "This company is already a customer — draft auto-rejected.", { code: "existing_customer" });
  }
