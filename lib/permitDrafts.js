...
export async function generatePermitDrafts(pool, { cap = 100, operatorKeys = null } = {}) {
  ...
  // Pre- vs post-expiry copy, see lib/permitCopy.js.
  const _picked = choosePermitCopy(tplRow);
  const tpl = tplRow ? { ...tplRow, subject: _picked.subject, body_html: _picked.body_html } : null;
...
export async function approveAndSendPermitDraft(pool, id) {
  ...
  const mRes = await pool.query(
    `SELECT id, email, apollo_mailbox_id, permit_enabled, warmup_started_at, daily_send_limit
  ...
  // Same gate as SDR outreach: the permit lane sends from these same inboxes, so it has to
  // respect the same bounce throttle or it re-burns the reputation the throttle is protecting.
  const health = (await mailboxBounceHealth(pool, { mailboxId: mailbox.id })).get(mailbox.id);
  const cap = dailyCap(mailbox.warmup_started_at, { target: mailbox.daily_send_limit, health });
...
export async function sendPermitToOperator(pool, op, mailbox, tpl) {
  ...
  // Per-inbox warmup cap (shared counter), held back by the mailbox's own limit and its
  // recent bounce rate — same gate the SDR lane uses, same inboxes.
  const health = (await mailboxBounceHealth(pool, { mailboxId: mailbox.id })).get(mailbox.id);
  const cap = dailyCap(mailbox.warmup_started_at, { target: mailbox.daily_send_limit, health });
  const sentToday = await mailboxSentTodayShared(pool, mailbox.id);
