...
export async function findEmailsForPromoted(pool, { cap = 25, operatorKeys = null } = {}) {
  ...
  // When a specific set is passed (multi-select), probe exactly those (still skipping any
  // already probed, so we never re-spend); otherwise the top promoted by score.
