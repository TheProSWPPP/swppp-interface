...
// A bounce / non-delivery report / auto-responder is NOT a real reply — never forward it to a
// rep and never spawn a "reply received, follow up" task off it. NDRs and auto-replies often
// quote the original recipient, so they match a lead by participant; we catch them here by the
// daemon From address and the standard NDR / vacation subject + body phrasing.
function isBounceOrAuto(from, subject, snippet) {
  const f = String(from || "").toLowerCase();
  if (/mailer-daemon|postmaster@|mail delivery (subsystem|system)|@.*\.(mailer-daemon)/i.test(f)) return true;
  const s = `${subject || ""} ${snippet || ""}`.toLowerCase();
  return (
    /delivery status notification|undeliverable|mail delivery (failed|subsystem)|returned mail|delivery (has )?failed|delivery incomplete|failure notice|message (was )?not delivered|address (not found|couldn'?t be found)|recipient.*not found|could ?n'?t be delivered|wasn'?t delivered|message blocked|550 5\.|automatic reply|auto[- ]?reply|out of (the )?office|on vacation|away from (my|the) (desk|office)/i.test(
      s,
    )
  );
}
