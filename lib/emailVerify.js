...
// Closed canonical verdict vocabulary every provider's raw result — and every one of OUR OWN
// DB writes of a verdict — must collapse into before it's used in a gate or persisted. Three
// different provider vocabularies (see file header) otherwise leak into gates/columns written
// assuming one specific provider (the exact bug this closes: a denylist hard-coded to
// NeverBounce's `invalid|disposable` silently let ZeroBounce's `spamtrap|abuse|do_not_mail`
// through). Idempotent by design — canonicalVerdict() of an already-canonical value (or of a
// column that mixes historical raw strings with new canonical ones) returns itself, so no data
// migration is required when this function is introduced or a provider is swapped later.
//   valid     — deliverable, safe to send/enroll
//   soft      — deliverable-but-uncertain (catch-all / unknown) — allowed through, not a block
//   hard_fail — confident-bad — block
//   skipped   — no usable verdict (no key, provider "error", or an unrecognized string) —
//               fail-open: never gate on this, never treat as a block
const VERDICT_MAP = {
  // MillionVerifier: ok | catch_all | unknown | invalid | disposable | error
  ok: "valid",
  catch_all: "soft",
  invalid: "hard_fail",
  ...
  unknown: "soft",
  // idempotent self-mappings — a value that's already canonical stays canonical
  soft: "soft",
  hard_fail: "hard_fail",
  skipped: "skipped",
};

/** Normalize any provider's raw verdict string (or an already-canonical one) to the closed
 * { valid | soft | hard_fail | skipped } vocabulary. Unrecognized/empty input → "skipped"
 * (fail-open — never guess a block). Case/whitespace-insensitive. */
export function canonicalVerdict(raw) {
  if (raw === null || raw === undefined || raw === "") return "skipped";
  const key = String(raw).toLowerCase().trim();
  return VERDICT_MAP[key] || "skipped";
}
