...
import { permitNbVerifyEnabled, verifyNewlyRevealedEmails } from "./permitNbVerify.js";
import {
...
async function getCrossRefSets(pool) {
  ...
  try {
    // `name` is the PROJECT name; the company we did the work for lives in data->>'companyName'.
    // An operator name can only ever equal the latter, so index that and not the project title.
    const { rows } = await pool.query(
      `SELECT data->>'companyName' AS company FROM projects
        WHERE archived = false AND data->>'companyName' IS NOT NULL`
    );
    custSet = crossRefIndex(rows, (r) => r.company);
  } catch (e) {
...
export function registerPermitRoutes(app, pool) {
  ...
  //
  // Optional NeverBounce follow-up (item D, flag PERMIT_NB_VERIFY, default OFF): writes
  // nb_result on the freshly-probed rows so a new find never sits at nb_result IS NULL
  // (silently "sendable" per lib/permitAuto.js:43) with no live verdict ever run against
  // it. Disabled path is a strict no-op — see lib/permitNbVerify.js.
  app.post("/api/permits/find-emails", async (req, res) => {
    ...
    try {
      // Captured BEFORE probing (via the DB's own clock, with a small safety buffer) so the
      // nb-verify follow-up scopes to exactly the rows this request just wrote, without
      // needing findEmailsForPromoted() to return operator_keys.
      let probedSince = null;
      if (permitNbVerifyEnabled()) {
        const t = await pool.query(`SELECT NOW() - INTERVAL '2 seconds' AS t`);
        probedSince = t.rows[0].t;
      }
      const result = await findEmailsForPromoted(pool, { cap: req.body?.cap, operatorKeys: req.body?.operator_keys });
      if (probedSince) {
        try {
          result.nbVerify = await verifyNewlyRevealedEmails(pool, { probedSince });
        } catch (e) {
          console.error("[permit-nb-verify] run failed:", e.message);
          result.nbVerify = { error: e.message };
        }
      }
      res.json(result);
    } catch (err) {
  ...
  //
  // `permit_enabled` is an arming flag — lib/permitAuto.js:enabledMailboxes() selects on
  // `permit_enabled = TRUE AND apollo_mailbox_id IS NOT NULL`, with NO `active` check, and the
  // daily permit-drip cron sends from whatever that query returns. Two mailboxes (pm@, sr@) are
  // deliberately `active = false` so a human decides when they go live. This route previously had
  // no `active` check of its own, so a single toggle click in the (also ungated) UI switch could
  // flip permit_enabled = true on an inactive mailbox and have it sending inside the hour — the
  // exact class of bug fixed for the Apollo-sync path in d964b86, left open here.
  //
  // Refuse to arm (permit_enabled: true) an inactive mailbox. Turning a mailbox OFF
  // (permit_enabled: false) is always allowed, active or not — that direction can never cause a
  // send. Also require a non-empty permit_signature when arming: an empty/whitespace signature
  // would let the drip send with no sign-off, and every currently-active mailbox (dc/jg/mh/th)
  // already has one set, so this cannot break a legitimate enable — it only stops arming a
  // freshly-synced mailbox nobody has finished configuring yet.
  app.patch("/api/permits/mailboxes/:id", async (req, res) => {
    ...
    try {
      if (req.body.permit_enabled === true) {
        const cur = await pool.query(
          `SELECT id, email, active, permit_signature FROM sdr_mailboxes WHERE id = $1`,
          [req.params.id],
        );
        if (!cur.rowCount) return res.status(404).json({ error: "Mailbox not found" });
        const mb = cur.rows[0];
        if (!mb.active) {
          return res.status(409).json({
            code: "mailbox_inactive",
            error: `Mailbox ${mb.email} is not active — cannot enable permit sending. Activate it first (SDR → Mailboxes).`,
          });
        }
        if (!mb.permit_signature || !mb.permit_signature.trim()) {
          return res.status(409).json({
            code: "missing_signature",
            error: `Mailbox ${mb.email} has no permit_signature set — cannot enable permit sending without a sign-off.`,
          });
        }
      }
      const r = await pool.query(
