...
/** Map aggregated operator row to stage label. */
...
export function registerPermitRoutes(app, pool) {
  ...
  app.get("/api/permits/operators-list", async (req, res) => {
    ...
    try {
      ...
      // Stage filter (pool/promoted/enriched in HAVING; mailed is post-filtered in JS)
      if (stage === "pool") {
        having.push(`BOOL_OR(f.status IN ('promoted','enriched')) = false`);
      } else if (stage === "promoted") {
        having.push(`BOOL_OR(f.status = 'promoted') = true AND BOOL_OR(f.status = 'enriched') = false`);
      } else if (stage === "enriched") {
        having.push(`BOOL_OR(f.status = 'enriched') = true`);
      }
      // stage='mailed' is post-filtered in JS (needs outreach subquery result)
      ...
      // Post-filter stage='mailed'
      if (stage === "mailed") {
        operators = operators.filter((r) => r.mailed);
      }
      ...
      let counts = { pool: 0, promoted: 0, enriched: 0, mailed: 0 };
      try {
        const funnelSql = `
          SELECT f.operator_key,
                 BOOL_OR(f.status = 'enriched') AS any_enriched,
      ...
      let total = Number(countResult.rows[0].n);
      // For stage='mailed', the count query can't know the post-filter result — use operators.length as best effort
      // (or fall through; the count for mailed will over-count, which is acceptable for pagination)

      res.json({ operators, total, page, pageSize, counts });
    } catch (err) {
  ...
  app.post("/api/permits/operator/:operatorKey/outreach", async (req, res) => {
    ...
    try {
      const result = await pool.query(
        ...
        [req.params.operatorKey, channel || "mail", status, note || null]
      );
  ...
  app.get("/api/permits/export/direct-mail.csv", async (req, res) => {
    ...
    try {
      ...
      if (mailedKeys.length > 0) {
        const batchId = new Date()
          .toISOString()
          .replace(/[-:T]/g, "")
          .slice(0, 14); // YYYYMMDDHHmmss
        // Use unnest for a single-round-trip bulk insert
        await pool.query(
          `INSERT INTO permit_outreach (operator_key, channel, status, batch_id)
           SELECT unnest($1::text[]), 'mail', 'exported', $2`,
          [mailedKeys, batchId]
        );
      }
