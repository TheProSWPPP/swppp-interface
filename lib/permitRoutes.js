...
async function getCrossRefSets(pool) {
  ...
  try {
    ...
    // An operator name can only ever equal the latter, so index both and union them.
    const { rows } = await pool.query(
      `SELECT name, data->>'companyName' AS company FROM projects WHERE archived = false`
    );
    custSet = crossRefIndex(rows, (r) => r.company);
    for (const k of crossRefIndex(rows, (r) => r.name)) custSet.add(k);
  } catch (e) {
