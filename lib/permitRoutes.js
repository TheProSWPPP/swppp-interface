...
/** Map aggregated operator row to stage label. Email-first: a found email
 *  outranks a mailing address (email is the priority channel). */
function operatorStage(row) {
  if (row.mailed) return "mailed";
  if (row.emailable) return "email_ready";
  if (row.any_enriched) return "enriched";
...
export function registerPermitRoutes(app, pool) {
  ...
  app.get("/api/permits/operators-list", async (req, res) => {
    ...
    try {
      ...
      // Stage filter (all in HAVING via subquery-based conditions). Email-first +
      // mutually exclusive, so the chips match the funnel counts: a found email
      // makes an operator "email_ready" regardless of whether it also has an address.
      ...
      if (stage === "pool") {
        ...
        having.push(`BOOL_OR(f.status = 'promoted') = true AND BOOL_OR(f.status = 'enriched') = false AND NOT ${HAS_EMAIL}`);
      } else if (stage === "email_ready") {
        having.push(HAS_EMAIL);
      } else if (stage === "enriched") {
        having.push(`BOOL_OR(f.status = 'enriched') = true AND NOT ${HAS_EMAIL}`);
      ...
      // Email-first + mutually exclusive (matches operatorStage precedence).
      let counts = { pool: 0, promoted: 0, email_ready: 0, enriched: 0, mailed: 0 };
      try {
        const funnelSql = `
          SELECT f.operator_key,
                 BOOL_OR(f.status = 'enriched') AS any_enriched,
        ...
        for (const r of funnelResult.rows) {
          ...
          else if (r.any_enriched) counts.enriched++;
  ...
  // GET /api/permits/export/direct-mail.csv — operator-deduped direct-mail list
  // One row per operator_key (DISTINCT ON), best contact + mailable address.
  // Logs each exported operator_key as permit_outreach status='exported'.
  // ?includeContacted=1 to include operators already exported/mailed.
  app.get("/api/permits/export/direct-mail.csv", async (req, res) => {
    if (!process.env.DATABASE_URL) return res.status(503).json({ error: "Database not configured" });
    try {
      const includeContacted = req.query.includeContacted === "1";

      const contactedFilter = includeContacted
        ? ""
        : `AND NOT EXISTS (
             SELECT 1 FROM permit_outreach o
              WHERE o.operator_key = f.operator_key
                AND o.status IN ('exported','mailed')
           )`;

      const { rows } = await pool.query(
        `SELECT DISTINCT ON (f.operator_key)
                f.operator_key, f.external_permit_nmbr, f.operator_name,
                to_char(f.expiration_date, 'YYYY-MM-DD') AS expiration_date,
                e.contact_name, e.mailing_address, f.city
           FROM permit_enrichment e
           JOIN permit_facilities f ON f.external_permit_nmbr = e.external_permit_nmbr
          WHERE e.channel = 'mail' AND e.mailing_address IS NOT NULL
                ${contactedFilter}
                -- Email-first: skip operators we have an email for (they go via the Email Queue,
                -- so the mailing list stays mail-only and we never double-contact).
                AND NOT EXISTS (
                  SELECT 1 FROM permit_operator_email pe
                   WHERE pe.operator_key = f.operator_key
                     AND pe.email IS NOT NULL AND pe.email <> ''
                )
          ORDER BY f.operator_key,
                   (e.contact_name IS NOT NULL) DESC,
                   f.score DESC`
      );

      // Build CSV first (buildDirectMailCsv filters non-mailable rows internally)
      const csv = buildDirectMailCsv(rows);

      // Collect the operator_keys that made it into the CSV (mailable rows only)
      // We re-check mailability here so the outreach log matches what was sent
      const mailedKeys = rows
        .filter((r) => /\d{5}/.test(String(r.mailing_address || "")))
        .map((r) => r.operator_key);

      // Bulk-insert outreach log entries for each exported operator.
      // (a) Skip logging entirely on re-export/preview (?includeContacted=1).
      // (b) Only insert for operator_keys that don't already have an 'exported' or 'mailed' row
      //     so re-downloads never create duplicate 'exported' rows.
      if (!includeContacted && mailedKeys.length > 0) {
        const batchId = new Date()
          .toISOString()
          .replace(/[-:T]/g, "")
          .slice(0, 14); // YYYYMMDDHHmmss
        // INSERT via SELECT with NOT EXISTS guard — one round-trip, no duplicates
        await pool.query(
          `INSERT INTO permit_outreach (operator_key, channel, status, batch_id)
           SELECT k, 'mail', 'exported', $2
             FROM unnest($1::text[]) AS k
            WHERE NOT EXISTS (
              SELECT 1 FROM permit_outreach o
               WHERE o.operator_key = k
                 AND o.status IN ('exported','mailed')
            )`,
          [mailedKeys, batchId]
        );
