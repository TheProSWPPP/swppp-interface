...
// Existing customers, keyed for soft name matching. `projects.name` holds the PROJECT name and
// `data->>'companyName'` holds the company we actually did the work for, so both are indexed:
// the company name is the one that can equal an operator name, the project name is kept because
// the UI's possible_customer flag has always used it.
async function customerKeys(pool) {
  try {
    const { rows } = await pool.query(
      `SELECT name, data->>'companyName' AS company FROM projects WHERE archived = false`,
    );
    const keys = crossRefIndex(rows, (r) => r.company);
    for (const k of crossRefIndex(rows, (r) => r.name)) keys.add(k);
    return keys;
  } catch (e) {
