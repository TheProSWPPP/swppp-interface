...
// Existing customers, keyed for soft name matching. ONLY `data->>'companyName'` is indexed:
// that is the company we did the work for, and the only field an operator name can equal.
// `projects.name` is the PROJECT name and is deliberately excluded — it yields 423 extra keys
// including junk like "kfc", "circlek", "home", "page", "sand pit" and "asdfasdf". A permit
// skip is written to permit_outreach, which has no delete path, so a false match here kills a
// real operator permanently and silently. Display code may be loose; a kill path may not.
async function customerKeys(pool) {
  try {
    const { rows } = await pool.query(
      `SELECT data->>'companyName' AS company FROM projects
        WHERE archived = false AND data->>'companyName' IS NOT NULL`,
    );
    return crossRefIndex(rows, (r) => r.company);
  } catch (e) {
    // Never let a cross-ref failure stop the drip; an empty set just means no suppression.
    console.error("[permit-auto] customer cross-ref failed:", e.message);
    return new Set();
  }
}
...
export async function runPermitAutoOutreach(pool) {
  ...
  const custKeys = await customerKeys(pool);

  let sent = 0, skippedBad = 0, skippedWeak = 0, skippedCustomer = 0, mi = 0;
  const errors = [];
  for (const op of candidates) {
    if (sent >= totalBudget) break;
    // Already a customer: never cold-pitch a renewal to someone we already do this work for.
    // Logged as skipped so the operator drops out of the pool instead of being retried daily.
    if (custKeys.has(companyKey(op.operator_name))) {
      await pool.query(
        `INSERT INTO permit_outreach (operator_key, channel, status, note) VALUES ($1,'email','skipped',$2)`,
        [op.operator_key, `auto: skipped existing customer (${op.operator_name})`],
      );
      skippedCustomer++;
      continue;
    }
  ...
  return { sent, skippedBad, skippedWeak, skippedCustomer, candidates: candidates.length, errors: errors.slice(0, 5) };
}
