...
export async function pollEngagement(pool, { baseUrl, callbackSecret, force = false } = {}) {
  ...
  try {
    ...
    const emit = async (ev) => {
      try {
        ...
        if (r.ok) {
          ...
          // A non-2xx used to vanish here. On 2026-07-28 every insert of the backfill sweep
          // returned 500 (the process_status CHECK rejected 'backfilled'), this branch dropped
          // all of them, and the watermark below still closed — so the next full scan replayed
          // 5-to-20-day-old events through the live side-effect path. Count and log it.
          emitFailures++;
          if (emitFailures <= 3) {
            console.error(`[engagement-poll] ingest rejected ${ev.type} with HTTP ${r.status} (${await r.text().catch(() => "")})`.slice(0, 400));
          }
        }
      } catch (e) {
        // Transport failure. Still a failure for watermark purposes: an unrecorded event is
        // unrecorded whether the server said no or never answered.
        emitFailures++;
        if (emitFailures <= 3) console.error(`[engagement-poll] ingest emit failed: ${e.message}`);
      }
    };
    ...
    //
    // A sweep is also partial when it read everything but PERSISTED nothing. The original
    // condition only covered read-side truncation (rate limit, budget), so the 2026-07-28 sweep
    // — which walked every sequence and had all 35 of its inserts rejected by a CHECK constraint
    // — passed this gate and closed the window on history it had never recorded. `emitFailures`
    // closes that hole: in backfill mode the window stays open unless every emit landed, which
    // repeats the sweep, which is the safe direction. `engagement_last_full_scan_at` still
    // stamps either way, because that only drives the 3h tier and a repeated full scan is fine.
    let watermark = null;
    if (fullScan && !rateLimited) {
      const closeBackfill = shouldCloseBackfillWindow({ backfillMode, emitFailures });
      try {
        const { rows: wm } = await pool.query(
          `UPDATE sdr_settings
              SET engagement_last_full_scan_at = NOW(),
                  engagement_backfill_done_at  = CASE WHEN $2::boolean
                                                      THEN COALESCE(engagement_backfill_done_at, NOW())
                                                      ELSE engagement_backfill_done_at END
            WHERE id = $1
        RETURNING engagement_backfill_done_at, engagement_last_full_scan_at`,
          [BACKFILL_SETTINGS_ID, closeBackfill],
        );
        watermark = {
          backfill_done_at: wm[0]?.engagement_backfill_done_at || null,
          last_full_scan_at: wm[0]?.engagement_last_full_scan_at || null,
          ...(backfillMode ? { backfill_just_closed: closeBackfill, emit_failures: emitFailures } : {}),
        };
        if (backfillMode && closeBackfill) {
          console.log(
          ...
          console.error(
            `[engagement-poll] backfill sweep had ${emitFailures} failed emit(s) — watermark deliberately NOT stamped, sweep repeats next full scan. Fix the ingest error before this closes.`,
          );
        }
      } catch (e) {
