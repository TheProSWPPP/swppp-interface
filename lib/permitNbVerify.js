...
import { verifyEmail, activeProvider } from "./emailVerify.js";
...
export function permitNbVerifyEnabled() {
  ...
  // Gate on the provider that will ACTUALLY be resolved, never on key presence. resolveProvider()
  // in emailVerify.js prefers MillionVerifier when several keys are set, and MillionVerifier
  // returns "ok" for a good address. Writing "ok" into nb_result would fail the send gate
  // (nb_result IN ('valid','unknown')) and drop a deliverable operator from the pool for good.
  const { provider, hasKey } = activeProvider();
  return provider === "neverbounce" && hasKey;
}
