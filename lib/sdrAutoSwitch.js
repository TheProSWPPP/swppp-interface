...
//
// OFF by default. Enable with SDR_AUTO_SWITCH=on. Disable instantly by unsetting it.
...
export function autoSwitchEnabled() {
  return process.env.SDR_AUTO_SWITCH === "on";
}
...
export async function runAutoSwitch(pool, { enrollDrafts }) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (!triggerChanged && !personChanged && !orgChanged) continue;
