...
//
// Ordering is RESOLVE-then-stop-then-enroll. We work out what the new enrolment would be and
// whether it is a duplicate BEFORE touching the live sequence, then remove the contact from the
// old Apollo sequence, clear Sequence_Started and mark the send 'switched', then enroll.
//
// Resolving first is load-bearing, not tidiness. Stopping first stranded 13 leads in the 30 days
// to 2026-08-06: every one had the same trigger before and after, a person_id or org_id that had
// flipped to a Pipedrive duplicate record, and a prior send of that same sequence to that same
// address. The engine stopped a working drip, then the duplicate guard correctly refused to
// re-enrol, and the lead sat out of outreach with nothing scheduled. Nothing had actually
// changed except a Pipedrive row id.
//
// So a hop that resolves to "we have already sent this exact sequence to this exact person"
// now leaves the enrolment running untouched. The old sequence is still dead before a new one
// starts in every case where we do switch, so this never double-touches a lead.
//
 ...
 * @param deps.buildDraft   seam for tests; defaults to the real generator.
 */
export async function runAutoSwitch(pool, { enrollDrafts, buildDraft = buildDraftFromLead }) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      // Stop the live sequence: remove from Apollo, clear the Pipedrive marker, mark the send
      // switched. Only called once we know a switch is actually going to happen.
      const stopEnrolment = async (note) => {
        try {
        ...
        try {
          if (process.env.PIPEDRIVE_API_TOKEN) {
            ...
            await pipedriveClient.addNote({ leadId: r.pipedrive_lead_id, content: note });
          }
        } catch (e) {
      ...
      // Nothing to re-enroll into (unsequenced stage, or no mailbox owner to send as). The
      // enrolled drip no longer matches the lead, so stop it and leave a note for a human.
      if (!VALID_TRIGGERS.has(newTrigger) || !r.owner_user_id) {
        await stopEnrolment(
          `[Auto] Sequence stopped: ${reasons}. No sequence available for the new state — needs manual follow-up.`,
        );
      ...
      try {
        ...
        // Checked BEFORE the stop: when the answer is "already sent", the honest conclusion is
        // that nothing changed for the recipient, so the running drip is still the right one
        // and gets left alone. Stopping first is what stranded 13 leads (see the file header).
        ...
        // Thrown before stopEnrolment (building the draft) leaves the lead on its existing
        // drip and the next sweep retries it — better than stranding it. Thrown after, the
        // lead is stopped with the note above and needs a manual enroll.
        reEnrollFailed++;
