  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (r.outcome === "ok") {
        ...
        await writeVerifyCache(pool, lead.leadId, { status: "invalid", verifiedValue: email, resolvedEmail: r.email, flag: null });
        stats.recovered++;
      } else {
        ...
        try {
          await pipedriveClient.addNote({
            leadId: lead.leadId,
            content: `[Auto] Email ${email} failed verification and no working alternate was found — flagged email_bad, in-flight outreach cancelled.`,
          });
        } catch (e) {
          console.error(`runVerificationPass: flag note failed for lead ${lead.leadId}`, e.message);
        }
        stats.flagged++;
      }
    } catch (e) {
