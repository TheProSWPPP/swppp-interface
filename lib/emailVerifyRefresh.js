...
import { canonicalVerdict } from "./emailVerify.js";
...
export async function verifyOneLead(pool, lead, deps) {
  ...
  if (r.outcome === "ok") {
    ...
    // Write the CANONICAL verdict, not the raw provider string — v.status is whichever
    // provider's own vocabulary (MillionVerifier/NeverBounce/ZeroBounce differ; see
    // lib/emailVerify.js). A raw write here was harmless only by coincidence (NeverBounce
    // happens to resolve today); canonicalVerdict() makes it correct for any resolved provider,
    // and is idempotent so mixing with historical raw values already in the column is safe.
    // "skipped" (no verdict — verify() was fail-open) preserves the pre-existing "valid"
    // fallback, since resolveContact() only reaches outcome:"ok" when the result wasn't a
    // confident hard-fail.
    const canon = canonicalVerdict(v && v.status);
    const status = canon === "skipped" ? "valid" : canon;
