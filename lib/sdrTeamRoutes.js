...
// scripts/seed-sdr.mjs:22 does randomBytes(9) → base64 → strip [+/=] → slice(0,12),
// which yields a SHORTER string whenever the base64 happens to contain + or / (it
// produced a 10-char password on the very first gate run). base64url has no such
// characters, so this is a deterministic 16 chars / 96 bits. Hashing is unchanged:
// bcrypt at cost 10, byte-for-byte the same $2b$10$ format already in the table.
...
// has_password keys off password_set_at, NOT off password_hash. Every pre-existing row already
// carries a $2b$10$ hash written by scripts/seed-sdr.mjs from a random string nobody kept, and
// the column is NOT NULL, so a hash-based check reports "true" for four users who cannot actually
// log in. That makes the pre-flip safety gate for REQUIRE_PASSWORD vacuous: it can never fail.
// password_set_at is stamped only when a password is deliberately set through these routes.
const USER_COLS = `id, username, email, display_name, role, active, last_login_at, created_at, updated_at,
                   (password_set_at IS NOT NULL) AS has_password, password_set_at`;

// Idempotent; runs at registration, same pattern as ensurePermitDraftSchema.
async function ensureTeamSchema(pool) {
  await pool.query(`ALTER TABLE sdr_users ADD COLUMN IF NOT EXISTS password_set_at TIMESTAMPTZ`);
}

export function registerSdrTeamRoutes(app, pool) {
  ensureTeamSchema(pool).catch((e) =>
    console.error("[sdr-team] schema bootstrap failed:", e.message),
  );
  // ---------------------------------------------------------------- users
  ...
  app.post("/api/sdr/admin/users", async (req, res) => {
    ...
    try {
      ...
      const { rows } = await pool.query(
        `INSERT INTO sdr_users (username, email, password_hash, display_name, role, active, password_set_at)
  ...
  app.patch("/api/sdr/admin/users/:id", async (req, res) => {
    ...
    if (body.password !== undefined || body.generate_password === true) {
      ...
      sets.push(`password_set_at = NOW()`); // the only signal that a real password exists
    ...
    // Normalise exactly as the write above does: a raw `body.role === "sdr"` test lets
    // "SDR", "Sdr" and " sdr " through the guard and still writes role='sdr'.
    const demotingToSdr = body.role !== undefined && String(body.role).trim().toLowerCase() === "sdr";
    if (demotingToSdr || body.active === false) {
