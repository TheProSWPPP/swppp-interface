...
export async function runAutoOutreach(pool, { mailboxSentToday, maxDrafts } = {}) {
  ...
  // draft already, and the bid is recent — we don't auto-chase stale opportunities. AGC/LBA
  // triggers are post-bid by nature so their bid dates are always in the past; this keeps the
  // window to bids within AUTO_BID_RECENCY_DAYS (or future). Manual sends ignore this gate.
  const AUTO_BID_RECENCY_DAYS = 90;
  ...
  const { rows: leads } = await pool.query(
    `SELECT s.pipedrive_lead_id, s.trigger_type
      ...
      WHERE s.outreach_status = 'clear'
        ...
        AND s.bid_date >= CURRENT_DATE - INTERVAL '${AUTO_BID_RECENCY_DAYS} days'
        AND NOT EXISTS (
