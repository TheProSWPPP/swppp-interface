  ...
  const { rows: leads } = await pool.query(
    `SELECT s.pipedrive_lead_id, s.trigger_type
      ...
      WHERE (
        ...
        -- Don't retry a lead we recently FAILED to enroll or rejected. Apollo refuses some
        -- contacts outright (job-change flag) and the verifier rejects bad addresses —
        -- retrying every cycle just spams failed drafts and inflates the failure KPI. Skip for
        -- 30 days after the last failed/rejected attempt.
        --
        -- EXCEPT a contact refused because Apollo already had them in a campaign. That is
        -- transient, not a dead lead: these estimators bid constantly, the blocking sequence is
        -- a ~13-day walk, and once it finishes the enroll path releases the contact and this
        -- lead sends. Holding it the full 30 days buries the project past its own start date,
        -- which is how 413 leads went unreachable. Matched on the shared substring because
        -- Apollo's active/finished wording is not reliable. See lib/apolloCollision.js.
        AND NOT EXISTS (
          SELECT 1 FROM sdr_drafts d
           WHERE d.pipedrive_lead_id = s.pipedrive_lead_id
             AND d.status IN ('failed','rejected','cancelled')
             AND d.created_at > NOW() - (
                   CASE WHEN d.error_message LIKE '%in_other_campaigns%'
                        THEN INTERVAL '14 days' ELSE INTERVAL '30 days' END
                 )
        )
